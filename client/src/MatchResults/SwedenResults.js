...
class SwedenResults extends Component{
...
function renderMatchRows(resultmatches){
  ...
  if(resultmatches[i].team1==='SWE' ||resultmatches[i].team2==='SWE'){
    ...
    return (
      ...
      <Row className="match-cell" onClick={() => this.setState({ open: !this.state.open })}>
        <Col sm={2}>
          ...
          <Col sm={2}>
          <img src={images[this.props.abb1+'.png']} alt={''} width={35} height={25}/>
          <br />
         {this.props.t1}
        </Col>
        <Col sm={2}>
        ...
        <Col sm={2}>
          <img src={images[this.props.abb2+'.png']} alt={''} width={35} height={25}/>
          <br />
...
export default connect(mapStateToProps, mapDispatchToProps)(SwedenResults)
